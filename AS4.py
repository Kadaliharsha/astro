...
# Define the compatibility levels for preferred list
compatibility_levels = {
    6: "Extremely Favourable",
    1: "Favourable",
    3: "Favourable",
    5: "Favourable",  # Universal inclusion in compatible planets as "Favourable"
}

# Additional levels for other numbers
neutral_numbers = {2, 4}  # Example neutral numbers
least_favourable_numbers = {7, 8}  # Example least favourable numbers
enemy_numbers = {9}  # Example enemy numbers
...
class NumerologyApp:
    ...
    def get_owner_details(self):
        try:
            ...
            tk.Button(owner_details_window, text="Submit", command=lambda: self.process_owner_details(owner_details_window)).pack(pady=10)

        except ValueError:
            print("Please enter a valid number of owners")

    # Function to process owner details on submission
    def process_owner_details(self, owner_details_window):
        all_numbers = []  # To collect all PN and DN of all owners
        unique_numbers = set()  # To collect unique numbers (excluding duplicates)
        compatible_planet_numbers = []  # To collect compatible planet numbers

        for idx, owner_data in enumerate(self.owner_entries):
            # Access name and DOB safely
            if owner_data['name'] and owner_data['dob']:
        ...
        for planet in compatibility_table:
            ...
            if not any(num in planet["enemies"] for num in unique_numbers):
                ...
                if all(num in planet["friends"] or num in planet["neutrals"] for num in unique_numbers):
                    ...
                    compatible_planet_numbers.append(planet_number)  # Add planet number to compatible planet numbers

        # Always include Planet 5 as it is considered universal
        if 5 not in compatible_planet_numbers:
            compatible_planets.append("Planet 5: Mercury")
        
        print("Compatible Planets:")
        for planet in compatible_planets:
            print(planet)

        # Calculate compatibility levels for preferred numbers in unique numbers
        compatibility_summary = {}
        for number in unique_numbers:
            if number in compatibility_levels:
                compatibility_summary[number] = compatibility_levels[number]
            elif number in neutral_numbers:
                compatibility_summary[number] = "Neutral"
            elif number in least_favourable_numbers:
                compatibility_summary[number] = "Least Favourable"
            elif number in enemy_numbers:
                compatibility_summary[number] = "Enemy"
            else:
                compatibility_summary[number] = "Not Compatible"  # Default for non-preferred numbers

        print("Compatibility Levels for Numbers:")
        for number, level in compatibility_summary.items():
            print(f"Number {number}: {level}")

        # Now find name numbers based on KPN values and preferred list
        industry = self.selected_industry.get()
        industry_kpn = industry_kpn_mapping.get(industry, [])

        # Select preferred name number, prioritizing 6 if it's in PN/DN
        name_numbers = []
        if 6 in unique_numbers:
            name_numbers.append(6)
        else:
            # Add numbers from preferred list in order, if they match KPN
            name_numbers.extend([num for num in [1, 3, 5] if num in industry_kpn])

        print("Name Number:", name_numbers)

        # Close the owner details window
        owner_details_window.destroy()

# Function to find name numbers based on unique numbers and KPN
def find_name_numbers(unique_numbers, industry_kpn):
    name_numbers = [num for num in unique_numbers if num in industry_kpn]
    return name_numbers
