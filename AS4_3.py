...
class NumerologyApp:
    ...
    def process_owner_details(self):
        ...
        for idx, owner in enumerate(self.owner_entries):
            ...
            unique_numbers.update([primary_number, destiny_number])  # Add to unique set

            print(f"Owner {idx + 1} - Name: {name}, DOB: {dob}, PN: {primary_number}, DN: {destiny_number}")

        # Check compatibility and display results
        all_numbers = list(unique_numbers)  # Convert unique set to list
    ...
    def display_compatible_planets(self, all_numbers):
        ...
        for planet in compatibility_table:
            ...
            # Check if all numbers are in friends and neutrals, and no enemies
            is_compatible = (
    ...
    def display_compatible_planets_kpn(self, kpn, compatible_planets):
        ...
        for kpn_value in kpn:
            for planet in compatibility_table:
                if planet['planet_number'] == kpn_value:
                    # Check if KPN is compatible by ensuring no enemies and all in friends or neutrals
                    is_compatible = (
                        all(num in planet['friends'] or num in planet['neutrals'] for num in kpn) and
                        all(num not in planet['enemies'] for num in kpn)
                    )

                    if is_compatible and planet['planet_name'] in compatible_planets:
                        compatible_planets_kpn.append(planet['planet_name'])
